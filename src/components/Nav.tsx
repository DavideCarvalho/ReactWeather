import React, { Component, FormEvent } from 'react';
import { withRouter, RouteComponentProps, NavLink } from 'react-router-dom';

interface INavProps extends RouteComponentProps {
}

interface INavState {
  location: string;
}


class Nav extends Component<INavProps, INavState> {

  constructor(props: INavProps) {
    super(props);
    this.state = {
      location: '',
    };
    this.onSearch = this.onSearch.bind(this);
    this.changeLocation = this.changeLocation.bind(this);
  }

  onSearch({ location }: { location: string }) {
    return (e: FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      const encodedLocation: string = encodeURIComponent(location);
      if (!encodedLocation) return;
      this.props.history.push(`/?location=${encodedLocation}`);
      this.setState({
        location: ''
      });
    }
  }

  changeLocation(e: FormEvent<HTMLInputElement>) {
    this.setState({
      location: (e.target as HTMLInputElement).value,
    });
  }

  render() {
    return (
      <div className="top-bar">
        <div className="top-bar-left">
          <ul className="menu">
            <li className="menu-text">React Weather App</li>
            <li>
              <NavLink exact to="/" activeClassName="active" activeStyle={{ fontWeight: 'bold' }}>Get Weather</NavLink>
            </li>
            <li>
              <NavLink to="/about" activeClassName="active" activeStyle={{ fontWeight: 'bold' }}>About</NavLink>
            </li>
            <li>
              <NavLink to="/examples" activeClassName="active" activeStyle={{ fontWeight: 'bold' }}>Examples</NavLink>
            </li>
          </ul>
        </div>
        <div className="top-bar-right">
          <form onSubmit={this.onSearch({ location: this.state.location })}>
            <ul className="menu">
              <li>
                <input
                  type="search"
                  placeholder="Search weather by city"
                  onChange={this.changeLocation}
                  value={this.state.location}
                />
              </li>
              <li>
                <input type="submit" className="button" value="Get Weather" />
              </li>
            </ul>
          </form>
        </div>
      </div>
    );
  }
}

const navWithRouter = withRouter(Nav as any);
export { navWithRouter as Nav }